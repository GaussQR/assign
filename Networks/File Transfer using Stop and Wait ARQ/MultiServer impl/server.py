	...
	def run(self):
		...
		if result == True:
			...
			while True:
				...
				if frm == b'': break
				print(f'Message from {self.add}:', len(frm))
				# if len(frm) != FRAME_LEN:
				# 	print(frm)
				time.sleep(random.uniform(0, MAX_ST))
				new_sock.send(frm)
				try:
					integf = new_sock.recv(MAX_LEN)
					print(f'Message from {new_sock.getpeername()}:', integf)
...
if __name__ == '__main__':
	...
	if len(sys.argv) >= 3:
		...
		if len(sys.argv) >= 5: MAX_ST = float(sys.argv[4])
